%DebiasingRand_Marcotte_M7symmetric
% Process: Debiasing Auditor
% Data: Marcotte dataset
% Framework: M7 (F7)
% Manipulation: Balancing node degree bias in training, masking features
% with random numbers (method: r2), and accounting for learning asymmetricity
%01.09.2019
...
ParameterSelection = [1:5, 16:20];

%% Features
load Features_M3_MarcotteHH_r2 Features_Rand

%% Parameter optimization rounds
for paramterSet=ParameterSelection
    paramterSet
    ...
    for k=1:training_rounds
        ...
        postr=[POStr{k};POStr{k}(:,2),POStr{k}(:,1)];
        negtr=[NEGtr{k};NEGtr{k}(:,2),NEGtr{k}(:,1)];
        Tr=[postr;negtr];
...
%% Summary stats
GlobalTimeAmount = toc(GlobalTime)
